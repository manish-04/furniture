...
public class CartDAOImpl implements CartDAO {
	...
	public Cart getCartbyId(int pdid) {
		...
		return (Cart)this.sessionFactory.getCurrentSession().createQuery("from Cart where cid=:id").setInteger("id", pdid).list().get(0);
	...
	public void update(String user, String ship, String bill) {
		this.sessionFactory.getCurrentSession().createQuery("update Cart set shipping_Address = :ship, billing_Address= :bill where username=:name").setString("ship", ship).setString("bill", bill).setString("name", user).executeUpdate();
	}
